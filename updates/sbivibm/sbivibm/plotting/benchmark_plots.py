...
LOSS_DICT = {"elbo": r"$\mathcal{L}_{rKL}$", "forward_kl":r"$\mathcal{L}_{fKL}$", "fdivergence": r"$\mathcal{L}_{fKL}$ (fVB)",  r"renjey_divergence ($\alpha = $0.5)":r"$\mathcal{L}_{\alpha}$ ($\alpha = 0.5$)",  r"renjey_divergence ($\alpha = $0.5) (pathwise)":r"$\mathcal{L}_{\alpha}$ ($\alpha = 0.5$, STL)", "iwelbo":r"$\mathcal{L}_{IW}$", "na":"MCMC", "na (100 chains)":"MCMC (100 chains)", "iwelbo (pathwise)": r"$\mathcal{L}_{IW}$ (STL)", "elbo (mixture)": r"$\mathcal{L}_{rKL}$ (MoS)", "elbo (mixture) (implicit)": r"$\mathcal{L}_{rKL}$ (MoS, impl. grad)", "elbo (mixture_gauss) (implicit)": r"$\mathcal{L}_{rKL}$ (MoG, impl. grad)", "forward_kl (mixture) (implicit)": r"$\mathcal{L}_{fKL}$ (MoS)", "forward_kl (mixture_gauss) (implicit)": r"$\mathcal{L}_{fKL}$ (MoG)", "forward_kl (mixture)": r"$\mathcal{L}_{fKL}$ (MoS)",r"renjey_divergence ($\alpha = $0.5) (unbiased)": r"$\mathcal{L}_{\alpha}$ ($\alpha = 0.5$,unbiased)",r"renjey_divergence ($\alpha = $0.1)": r"$\mathcal{L}_{\alpha}$ ($\alpha = 0.1$)",r"renjey_divergence ($\alpha = $0.3)": r"$\mathcal{L}_{\alpha}$ ($\alpha = 0.3$)",r"renjey_divergence ($\alpha = $0.1) (pathwise)":r"$\mathcal{L}_{\alpha}$ ($\alpha = 0.1$,STL)", r"renjey_divergence ($\alpha = $0.1) (unbiased)":r"$\mathcal{L}_{\alpha}$ ($\alpha = 0.1$,unbiased)"}
GROUP_CATEGORIES = {"rKL": ["elbo"], "fKL":["forward_kl", "fdivergence"], "KL":["elbo", "forward_kl"], r"$\alpha$":["renjey_divergence"], "IW":["iwelbo"]}
LOSS_TO_COLOR = {"elbo": "#377eb8", "forward_kl":"#984ea3", "fdivergence":"#570052", r"renjey_divergence ($\alpha = $0.5)":"#4a0000", "iwelbo":"darkgreen", "na":"grey", "na (100 chains)":"black", "iwelbo (pathwise)":"#4daf4a",  "elbo (mixture)":"darkblue", r"renjey_divergence ($\alpha = $0.5) (pathwise)":"orangered", r"renjey_divergence ($\alpha = $0.1) (unbiased)":"darksalmon", r"renjey_divergence ($\alpha = $0.3)":"red", r"renjey_divergence ($\alpha = $0.1)":"indianred", "elbo (mixture_gauss) (implicit)": r"cornflowerblue", "forward_kl (mixture_gauss) (implicit)": r"purple", r"renjey_divergence ($\alpha = $0.1) (pathwise)":"#e41a1c",  "forward_kl (mixture)": "tab:pink", "elbo (mixture) (implicit)": "cyan", "forward_kl (mixture) (implicit)": r"darkslateblue"}
...
def plot_metrics(name:str, task:str, algorithm="NL", metrics:list=["c2st","mmd"], color_palette=None, groups=["KL", "IW", r"$\alpha$"], fontsize=11, title_fontsize=15, legend_anchor=(2, -0.6), legend_cols=6, legend=True, filter=None, height=None, linestyles=True):
    ...
    def pointplot(x,y,color, **kwargs):
        ...
        if len(idx_ir) > 0 and not linestyles:
            sns.pointplot(x=x[idx_ir],y=y[idx_ir], color=color,**kwargs)
        if len(idx_ir) > 0 and  linestyles:
            sns.pointplot(x=x[idx_ir],y=y[idx_ir], color=color,**kwargs)
        if len(idx_imh) > 0 and  linestyles:
    ...
    with sns.axes_style("whitegrid", rc={"font.size":fontsize,"axes.titlesize":title_fontsize,"axes.labelsize":fontsize}):
        ...
        g.fig.suptitle(TASKS[task], fontsize=title_fontsize+5,y=1.05)
        ...
        if legend:
            lines = []
            lines += [mlines.Line2D([0,1],[0,1],linestyle=':', color="black", label=r"$q_\phi$")]
            if mask_ir.sum() > 0:
                line = mlines.Line2D([0,1],[0,1], color="black", label=r"$q_\phi$ + SIR")
                lines.append(line)
            if mask_imh.sum() > 0:
