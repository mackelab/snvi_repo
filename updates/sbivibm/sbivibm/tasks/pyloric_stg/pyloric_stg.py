...
def slurm_simulator(thetas, simulation_batches = 500):
    ...
    for j in range(jobs):
        subprocess.run(["sbatch", DIR_PATH + os.sep  + "run_one.sh", DIR_PATH + os.sep  + f"thetas_{j}.pkl", f"{j}", DIR_PATH, f"{SEED_PER_ROUND}", f"--output={DIR_PATH}"])
...
class Pyloric(Task):
    ...
    def get_precomputed_dataset(self):
        df_paras = pd.read_pickle(str(Path(__file__).parent.absolute()) + "/files/all_circuit_parameters.pkl")
        df_simulation_output = pd.read_pickle(str(Path(__file__).parent.absolute()) +"/files/all_simulation_outputs.pkl")
